...
\newcommand{\outDate}{Monday, September 29}
\newcommand{\dueDate}{Wednesday, October 8 at 11:59 PM}
\newcommand{\taNames}{Santiago, Yuhan, Akhil, Joyce, Markov, Neural}
\newcommand{\homeworktype}{\string written+prog}
\newcommand{\exitPollLink}{\string https://forms.gle/7FK6FZfMGs9Bsage6}
...
\item \textbf{Collaboration Policy}: Please read the collaboration policy here: \url{https://www.cs.cmu.edu/~mgormley/courses/10601/index.html}

\item\textbf{Late Submission Policy:} See the late submission policy here: \url{https://www.cs.cmu.edu/~mgormley/courses/10601/index.html}
...
\begin{parts}
    ...
    \part[2] \sall Given that the negative log-likelihood loss is convex, which of the following statements are correct about training a logistic regression model using mini-batch SGD?
    
    {%
    \checkboxchar{$\Box$} \checkedchar{$\blacksquare$}
    \begin{checkboxes}
        \choice The learning algorithm might get stuck in a sub-optimal local minimum on small datasets.
        \choice Regardless of the learning rate, the algorithm can converge to the global minimum.
        \choice The learning algorithm might get stuck in a sub-optimal local minimum on large datasets.
        \choice  Regardless of the batch size, the algorithm can converge to the global minimum.
        \choice None of the above.
    \end{checkboxes}
    }
    ...
    You are training a logistic regression model on a dataset $\mathcal{D}_{train}$ with $M$ features and a binary target label $\yv \in \{0, 1\}$. The third feature in the dataset, $\xv_3$, linearly separates the training data. For all examples in the training dataset where $\xv_3 \geq 1$, the target label $y$ has a value of 1. The training dataset has examples with values of $\xv_3$ that span uniformly in the range [0, 2].
    
    \begin{subparts}
    
    \subpart[2]
    The following plots show how the predicted value of $\hat{P}(y=1)$ changes with different values of $\xv_3$. Which plot is most likely to represent the output of your model after training on $\mathcal{D}_{train}$ using SGD for many epochs ($>>100$)? \textbf{Explain your reasoning}.
    ...
    \subpart[2]    
    After training, you use a validation dataset $\mathcal{D}_{validation}$ to make the following plot.

    \begin{center}
        \includegraphics[width=6cm]{fig/Plot Validation.png}
    \end{center}

    You show your plot to your friend Neural, who suggests that you should add L2 regularization to your training objective. 
    
    \sone How would adding regularization change the weights of your model?

    
    {%
    
    \begin{checkboxes}
        \choice $\ell_2$ regularization prevents weights from going to infinity by normalizing all weights to have unit magnitude.
        \choice $\ell_2$ regularization prevents weights from going to infinity by reducing some of the weights to 0, effectively removing some of the features. 
        \choice $\ell_2$ regularization prevents weights from going to infinity by reducing the value of some of the weights to be \textit{closer} to 0 (reducing the effect of a feature but not necessarily removing it). 
        \choice None of the above.
    \end{checkboxes}
    }
